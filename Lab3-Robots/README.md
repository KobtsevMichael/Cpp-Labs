...
| . | – Пустая клетка, робот может по ней проехать. <br>
| # | – Клетка с камнем. Будет получена ошибка при попытке переместиться в эту клетку. <br>
| B | – Клетка с бомбой. Если робот заезжает на эту клетку, он уничтожается и игра заканчивается. <br>
| A | – Клетка с ресурсом, который необходимо собрать. <br>
...
ресурсов на карте и соствляет по ним максимально оптимальный путь
передвижения. При добавлении саппера в авто режиме саппер одновременно
с коллетором обезвреживает бомбы.

Ниже приведены примеры работы с саппером и без

```
SET_MODE AUTO - Активировать авто режим
SAPPER ON/OFF - Добавить/Убрать саппера с карты
```
